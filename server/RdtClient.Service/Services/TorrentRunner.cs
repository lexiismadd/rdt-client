...
{
    public class DockerDiskWatcher
    {
        private const int Seuil = 80;
        private const string NomConteneur = "rdtclient";
        private readonly DockerClient _dockerClient;
        private readonly ILogger<DockerDiskWatcher> _logger;

        public DockerDiskWatcher(DockerClient dockerClient, ILogger<DockerDiskWatcher> logger)
        {
            _dockerClient = dockerClient;
            _logger = logger;
        }

        public void SurveillerCapacite(object state)
        {
            try
            {
                DriveInfo driveInfo = new DriveInfo("/");
                double pourcentageUtilisation = ((double)driveInfo.TotalSize - driveInfo.AvailableFreeSpace) / driveInfo.TotalSize * 100;

                _logger.LogInformation($"Capacité totale du disque dur : {driveInfo.TotalSize / (1024 * 1024 * 1024)} Go");
                _logger.LogInformation($"Utilisation actuelle du disque dur : {driveInfo.UsedSize() / (1024 * 1024 * 1024)} Go");

                if (pourcentageUtilisation > Seuil)
                {
                    _logger.LogWarning($"La capacité du disque dur dépasse le seuil ({pourcentageUtilisation}% utilisé)");

                    // Arrêter le conteneur
                    _dockerClient.Containers.StopContainerAsync(NomConteneur, new ContainerStopParameters());

                    _logger.LogInformation($"Arrêt du conteneur {NomConteneur}");
                }
                else
                {
                    _logger.LogInformation($"Le conteneur {NomConteneur} redémarre");

                    // Redémarrer le conteneur
                    _dockerClient.Containers.RestartContainerAsync(NomConteneur, new ContainerRestartParameters());

                    _logger.LogInformation($"Redémarrage du conteneur {NomConteneur}");
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Une erreur s'est produite lors de la surveillance de la capacité du disque dur.");
            }
        }
    }

    public class DockerDiskWatcherTimer
    {
        private Timer _timer;
        private readonly DockerDiskWatcher _diskWatcher;

        public DockerDiskWatcherTimer(DockerDiskWatcher diskWatcher)
        {
        ...
        public void Start()
        {
            _timer = new Timer(_diskWatcher.SurveillerCapacite, null, TimeSpan.Zero, TimeSpan.FromSeconds(5));
        }
    ...
    {
        public static long UsedSize(this DriveInfo driveInfo)
        {
